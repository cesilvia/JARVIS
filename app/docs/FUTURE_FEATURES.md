# Future Features

Ideas and planned features for JARVIS, organized by module.

---

## Nutrition

- [ ] Recipe search with macro filtering
- [ ] Camera-based barcode scanning
- [ ] Meal planning features
- [ ] Connect to Convex backend for data persistence

## Cycling / Strava

- [ ] Activity feed on hub or bike page
- [ ] Route suggestions using Strava routes/segments
- [ ] Weekly/monthly mileage goals and targets
- [ ] Workout upload — push planned workouts to Strava
- [ ] Segment efforts — track PRs on favorite segments
- [ ] Power curve comparison across time periods

## Settings

- [ ] Password strength indicator on change password form
- [ ] Session management — view/revoke active sessions
- [ ] Export all app data (not just nutrition) as a single backup

## General

- [ ] Push notifications for alerts (zone review reminders, etc.)
- [ ] Dark/light theme toggle
- [ ] Mobile-optimized layouts for all pages
